...
country_number = [c for c in countries_data]

print("total number countries:",len(country_number))

# total number of Asian countries

asian_countries = [a.get("country_name") for a in countries_data if a.get("region") == "Asia"]

print("asian countries: ",len(asian_countries))

# country with highest population

#approach 1

population_countries = [c.get("population") for c in countries_data]

highest_population = [c.get("country_name") for c in countries_data if c.get("population") == max(population_countries)]
...
#approach 1
country_sorted = sorted(countries_data,key=lambda c:c.get("population"))
country_pop = {c.get("country_name"):c.get("population") for c in country_sorted}
print(country_pop)
print("---------------")

#approch 2
country_sorted1 = sorted(
    [(c.get("country_name"), c.get("population")) for c in countries_data],key=lambda x: x[1]
)

print(country_sorted1)

# display all unique region

unique_region = {c.get("region") for c in countries_data}
print(unique_region)
