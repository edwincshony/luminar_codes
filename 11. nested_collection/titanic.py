...
survived_ppl = [di for di in titanic_data if di.get("survived") == 1]

print(f"Survived people: {len(survived_ppl)}")

# q2 : display unique passenger class 

unique_class = {di.get("pclass") for di in titanic_data}

print(f"Unique passenger class: {unique_class}")

# q3 number of female passengers

female_passengers = [di for di in titanic_data if di.get("sex") == "female"]

print(f"Female passengers count: {len(female_passengers)}")

# q4: number of survived childs 

survived_child = [di for di in titanic_data if di.get("survived") == 1 and di.get("age") < 12]

print(f"Survived children count: {len(survived_child)}")

# q5: name whose fare > 30

fare_gt_thirty = [di.get("name") for di in titanic_data if di.get("fare") > 30]

print(fare_gt_thirty)

# q6: number survived female

survived_ppl_female = [di for di in titanic_data if di.get("survived") == 1 and di.get("sex") == "female"]

print(f"Survived female count: {len(survived_ppl_female)}")
...
survived_ppl_name = [di.get("name") for di in titanic_data if di.get("survived") == 1]

print(f"Survived people name: {survived_ppl_name}")
...
rate = round(len(survived_ppl_female)/len(female_passengers)*100,2)

print(f"female survival rate: {rate}")
